...
logger = logging.getLogger("dy-bridge")
...
class BridgeServer:
    ...
    async def _handle_cli(self, ws: ServerConnection, msg: dict) -> None:
        ...
        if not self._extension_ws:
            await ws.send(json.dumps({"error": "Extension 未连接，请确认浏览器已安装并启用 DY Bridge 扩展"}))
            return
...
async def main(port: int) -> None:
    server = BridgeServer()
    # 强制监听 127.0.0.1 避免 IPv6 域名解析问题
    async with websockets.serve(server.handle, "127.0.0.1", port, ping_interval=None, max_size=50 * 1024 * 1024):
        logger.info("Bridge server 已启动: ws://127.0.0.1:%d", port)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="DY Extension Bridge Server")
